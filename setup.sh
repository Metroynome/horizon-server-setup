...
configure_checkout() {
  ...
  python3 - "$INSTALL_ROOT" "$profile" "$SERVER_IP" "$INCLUDE_PLUGIN" "$INCLUDE_PATCH" "$INCLUDE_DNS" "$SECRETS_FILE" <<'PY'
import json, os, re, secrets, sqlite3, string, sys
from pathlib import Path

root = Path(sys.argv[1])
profile_path = Path(sys.argv[2])
server_ip = sys.argv[3]
include_plugin = sys.argv[4] == 'true'
include_patch = sys.argv[5] == 'true'
include_dns = sys.argv[6] == 'true'
secrets_path = Path(sys.argv[7])
...
horizon_docker = root / 'horizon-docker'
horizon_dns = root / 'horizon-dns'
required_checkouts = [horizon_docker]
if include_dns:
...
db_name = database.get('name') or 'Medius_Database'
...
if appsettings_path.exists():
    ...
    appsettings.setdefault('AppGroups', [])
    if not any(group.get('Name') == group_name for group in appsettings['AppGroups'] if isinstance(group, dict)):
        appsettings['AppGroups'].append({'Name': group_name})
    default_settings = {
    ...
    apps = appsettings.setdefault('Apps', [])
    existing_apps = {app.get('Id') for app in apps if isinstance(app, dict)}
    ...
    locations = appsettings.setdefault('Locations', [])
    for app_id in app_ids:
        if not any(location.get('AppId') == app_id for location in locations if isinstance(location, dict)):
            locations.append({'Id': 40, 'AppId': app_id, 'Name': 'Battledome' if profile.get('id') == 'dl' else 'Aquatos'})
    channels = appsettings.setdefault('Channels', [])
    for app_id in app_ids:
        if not any(channel.get('AppId') == app_id for channel in channels if isinstance(channel, dict)):
            channels.append({
                'Id': 1,
                'AppId': app_id,
                'Name': 'CY00000000-00',
...
compose = re.sub(
    ...
    r'\1image: horizon-server-local\n\1build:\n\1  context: ../horizon-server',
    compose,
)
compose = re.sub(
    r'(?ms)^(\s*horizon-middleware:\n)\s*image:\s*horizon-server-local\s*\n\s*build:\s*\n\s*context:\s*\.\./horizon-server-database-middleware\s*\n',
    r'\1    image: horizonprivateserver/horizon-server-database-middleware\n',
    compose,
)
...
compose = re.sub(r'(?m)^  horizon_database_data(?:_[A-Za-z0-9_-]+)?:\s*$', f'  {volume_name}:', compose)
if f'{volume_name}:' not in compose:
    compose = compose.rstrip() + f'\n\nvolumes:\n  {volume_name}:\n'
elif not re.search(rf'^volumes:\s*\n(?:  .+\n)*  {re.escape(volume_name)}:', compose, flags=re.MULTILINE):
    compose = compose.rstrip() + f'\n\nvolumes:\n  {volume_name}:\n'
